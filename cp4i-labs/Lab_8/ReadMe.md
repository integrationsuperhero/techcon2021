...
8\. For the this lab we will use MQ as one of our target applications.  We will need to login to the OCP console for your user id to set up the connection and security for the MQ QMgr.  
...
2\. Click on the es-demo to go to the Event Streams main page.
...
4\. Now enter the name of your topic.  Since this is a shared kafka broker use your userid as part of the topic name.  In this example we are login as wedge14 so the topic name is **wedge14.mytopic**  Click Next.
...
6\. For the Message retention since this is for a lab change to **A day** and click Next

![alt text][pic35]

7\. For the Replicas we will change the Minimum in-sync replicas to 1 and select Replication factor:1.  Then Click Topic
...
9\. Now for the Cluster connection make sure you select the External and then click the Generate SCRAM credentials.

![alt text][pic38]

10\. For the credentials name use your userid.  In this example it is wedge14.  Make sure the right button is selected then click Next.
...
13\. Select the Transactional IDs with prefix and use the same prefix.  Then click Generate credentials.
...
16\. Now we will go to App Connect Designer to setup conection to Event Steams.  In the upper left click on the IBM Automation to take you back to the home page. 

![alt text][pic30]

17\. Select the ace-desXX link.  
...
If not click on the Connect.

![alt text][pic45]

20\. For the Authorization method make sure to select **\(SASL_SSL\)** from the dropdown.  Then click Continue. 
...
4\. First thing we will do is give the flow a meaningful name.   In this example we can use **New SF Leads Events**.  Then we will select the connector to listen for events.  We will go to the Salesforce connecto and select the **New lead** under the Leads drop down. 
...
7\. You will now see the flow you just created for handling the SF lead events. Next we will click on the New to create the consumer flow.
...
9\. Now select the topic for your userid that you created in the last section.  And for the Group ID we will use your userid as the unique ID.  In this case we are using wedge14.   
...
12\. Now on the App Connect Designer Dashboard we should see are new flows we created.   We will need to start both of them by clicking on the 3-dots on each tile and selecting start.  
...
2\. In the left menu click on the 9-dots and the search window will open.  Type in Lead adn you will see Leads, Click on that.    

![alt text][pic60]


3\. You will now be on the Leads screen and on the top menu click on the New button.
...
7\. You will now see your queues including the one we created for this lab **SF.MYTOPIC.EVENT** and it should show at least 1 new message.  Click on the Queue name.  

![alt text][pic65]

8\. You will See the Timestamp when the message was put on the queue as well as the Appliaction Data we put on the queue. 
* Click on the message and a Message details will be displayed. 

![alt text][pic66]

9\. The Message details will should info from the MQ header as well as the Application data. 
